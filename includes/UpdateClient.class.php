...
class UpdateClient {
	...
	public function print_admin_scripts() {
		...
		if ($screen->base === 'plugins' || $screen->base === 'plugin-install') {
			...
			$text1 = esc_html__( 'Compatible up to:', 'endorse' );
			$text2 = esc_html__( 'Reviews', 'endorse' );
			$text3 = esc_html__( 'Read all reviews', 'endorse' );
	...
	public function filter_component_row_meta($component_meta, $component_file) {
		...
		if ($this->identifier === $component_file) {
			...
			$anchor_text = esc_html__( 'View details', 'endorse' );
